...
getRelationsInfo <- function(relation,MAF){
  ...
  write.table(finaluniqrelation, file = paste( "results/10_quality_filters_sex_relationship/UniqRelation_MAF_",MAF,".tsv" ,sep = ""), sep = "\t", quote = F, row.names = F   )
  ...
  write.table(PAIRS_related, file = paste( "results/10_quality_filters_sex_relationship/PairsRelation_MAF_",MAF,".tsv" ,sep = ""), sep = "\t", quote = F, row.names = F   )
}
